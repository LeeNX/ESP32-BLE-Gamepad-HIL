...
| GATT reads (`-m gatt`, `gatt.py`, bleak over CoreBluetooth / WinRT) | ✅ `test_gatt.py` — Device Info / PnP / Battery, matching `../host/hil/gatt.py`'s fields (BlueZ/D-Bus there). **Confirmed on macOS hardware, both directions**: these read fine even on an already-bonded, HID-owned device, but the HID service itself (0x1812 — Feature/Input/Output Report) is *not* exposed to a generic GATT client here — macOS hands that one service to its own Bluetooth HID stack once a HID-class peripheral connects, non-HID services stay reachable. `test_hid_service_not_reachable_via_gatt` pins it. Use Feature Report get/set (`-m hid`, `test_feature_report.py`) for that characteristic instead — it goes through the OS's native HID API, which *can* reach it. |
